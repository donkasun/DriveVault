 ...
 * `_FuelStatsCard` / `_FuelStatItem` (lines 738-912). Rows use `FuelRecordCard`
 * (swipe-to-delete, tap opens the quick fuel-entry sheet in edit mode).
 */
...
import { sortByDateDesc } from '../vehicle-detail-helpers';
  ...
  return (
    <View>
      ...
      {logsLoading ? null : logsError ? (
        ...
        <View>
          {recent.map((log) => (
            <FuelRecordCard key={log.id} log={log} vehicleId={vehicleId} />
          ))}
