...
import { updateVehicleSchema } from '@/server/schemas/vehicles';
import {
  deleteVehicle,
  getVehicleResponseForUser,
  updateVehicle,
...
export async function GET(req: NextRequest, context: RouteContext) {
  try {
    ...
    const data = await getVehicleResponseForUser(user.id, id);
...
export async function PATCH(req: NextRequest, context: RouteContext) {
  try {
    ...
    const payload = updateVehicleSchema.parse(body);
