...
- API base URL via `--dart-define=API_BASE_URL=...` (local vs Google Cloud Run prod).
...
- **Money:** store as integer **cents** (`amount_cents`) + a `currency` code — never floats.
  Currency is **hard-locked to `LKR`** app-wide (`LOCKED_CURRENCY = "LKR"` in
  `backend/app/core/constants.py`; enforced by migration `f3001_lock_currency_lkr`).
  No free-form multi-currency support exists in Phase 1.
- **Distance:** odometer, `interval_km`, and `current_mileage` are always stored in **integer km** in the DB and on the wire. Conversion to miles happens only on the client at the display/input edge using the user's (or vehicle's) `distance_unit` preference (`'km'`\|`'mi'`). Fuel economy is stored as L/100 km; km/L and mpg conversions are also client-side only. (Phase 1.5 user/vehicle preferences: `users.distance_unit` defaults to `'km'`; `vehicles.distance_unit` overrides per-vehicle, `NULL` = inherit user default.)
